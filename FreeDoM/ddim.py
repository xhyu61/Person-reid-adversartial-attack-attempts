...
from Reid_Block import Reid_Block
...
class ReID_DDIMSampler(object):
    def __init__(self, model, reid_block=None, schedule="linear", **kwargs):
        super().__init__()
        self.model = model
        self.ddpm_num_timesteps = model.num_timesteps
        self.schedule = schedule
        self.reid_block = reid_block
    ...
    def p_sample_ddim_conditional(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        for j in range(repeat):
            ...
            if start > index >= end:
                D_x0_t = self.model.decode_first_stage(pred_x0)      # <class 'torch.Tensor'>
                residual = self.image_encoder.get_gram_matrix_residual(D_x0_t) # torch.Size([768, 768])
                norm = torch.linalg.norm(residual)                   # 973 ~ 489, dtype: torch.float16
                ...
                D_x0_t2 = torch.clamp((D_x0_t + 1.0) / 2.0, min=0.0, max=1.0)
                D_x0_t2 = D_x0_t2.permute(0, 2, 3, 1).permute(0, 3, 1, 2)
                D_x0_torch = D_x0_t2.squeeze(dim=0)                    # torch.Size([3, 256, 256])
                
                # ------ evading 
                # identity_loss, camera_loss = self.reid_block.evading_loss(D_x0_torch)
                # camera_loss = self.reid_block.impersonating_loss(D_x0_torch)
                reid_loss = self.reid_block.impersonating_loss(D_x0_torch)
                # reid_loss = identity_loss + camera_loss
                total_loss = param_alpha * reid_loss + param_theta * norm
                
                wandb.log({'reid_loss': param_alpha * reid_loss})
                # norm_grad = torch.autograd.grad(outputs=norm, inputs=x)[0]
                norm_grad = torch.autograd.grad(outputs=total_loss, inputs=x)[0]
                # print(type(norm_grad), norm_grad.shape, norm_grad.sum())
                rho = (correction * correction).mean().sqrt().item() * unconditional_guidance_scale / (norm_grad * norm_grad).mean().sqrt().item() * 0.2
